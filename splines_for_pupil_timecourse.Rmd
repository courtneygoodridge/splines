...
ggsave(here::here("splines/Plots/pupil_timecourse_plot.tiff"), plot = pupil_timecourse_plot, width = 16, height = 8, units = 'cm', dpi = 300, type = 'cairo')
```
...
ggsave(here::here("splines/Plots/pupil_timecourse_spline_plot.tiff"), plot = pupil_timecourse_spline_plot, width = 16, height = 8, units = 'cm', dpi = 300, type = 'cairo')
```

# Interaction between continous and categorical variables 

Thus far, we have focused on the average pupil diameter of one condition (when N-back during automation was true, and for the most critical scenario). However, we might expect the timecourse of pupil diameter to change depending on whether drivers completed N-back during the 2 minutes of automation. If we average the data for when drivers completed N-back or not, and plot over time, we can see that there might be a difference in the timecourse in the first 10 s, but perhaps not so much after that
...
Model 1 (m1) aims to model a seperate smooth for each level of n_back. The *gratia* package has a function for plotting how it would be more benefical to try and plot this ourselves. This webpage is a good indication of how to do that (https://gavinsimpson.github.io/gratia/articles/custom-plotting.html). 

...
The *mgcv* package for fitting Generalised Additive Models (GAMs) to model non-linear data. They also allow for multiple level models, but we focus first on the fixed effects. 

Model 1 (m1) aims to model a seperate smooth for each level of n_back. The *"k"* argument in the *s()* function within the *gam()* specifies the number of basis functions. The documentation state that this should not restrictively small, nor too large (for computational issues). 

The *gratia* package has a function for plotting how it would be more beneficial to try and plot this ourselves. This webpage is a good indication of how to do that (https://gavinsimpson.github.io/gratia/articles/custom-plotting.html). 


From the Generalised Additive Model, we can plot what are called "partial effects". Partial effects  refer to the isolated change in the value of the response variable for every unit change in the predictor variable. 

Open questions:

- Are partial effects estimations of deviations from the intercepts (which are themselves the mean over the timecourse)?

Partial effects seem to reveal that although pupil diameter increases at approximately the same rate, the enlarged pupil diameter persists for longer when drivers have completed N-back during automation. 

*(https://towardsdatascience.com/producing-insights-with-generalized-additive-models-gams-cf2b68b1b847)* - might be useful. 

```{r}
# modelling average pupil diameter over time, for both levels of n-back 
m1 <- gam(diameter ~ n_back + s(time, by = n_back, k = 20),
...
ggplot(sm, mapping = aes(time, est, col = as.factor(n_back))) +
  geom_path()
